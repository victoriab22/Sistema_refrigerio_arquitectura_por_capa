...
async function enviarSolicitud(e) {
    ...
    try {
        const respuesta = await fetch("../../controladores/SolicitudController.php", {
            method: "POST",
            body: datos
        });
        let resultado;
        ...
        if (respuesta.ok && resultado.success) {
            ...
            while (detallesContainer.children.length > 1) {
                detallesContainer.removeChild(detallesContainer.lastChild);
            }
            const primerItem = detallesContainer.querySelector('.item-detalle');
            if (primerItem) {
                primerItem.querySelectorAll('input, select').forEach(campo => {
